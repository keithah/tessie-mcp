...
export class SessionRegistry {
  ...
  async expire() {
    ...
    const expired = [...this.sessions.entries()].filter(([, entry]) => entry.touched < cutoff);
    expired.forEach(([id]) => this.sessions.delete(id));
    await Promise.allSettled(expired.map(([, entry]) => entry.transport.close()));
  }

  async dispose() {
    clearInterval(this.reaper);
    const transports = [...this.sessions.values()].map(({ transport }) => transport);
    this.sessions.clear();
