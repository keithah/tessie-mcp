...
const paramsSchema = z.object({ charge_limit_percent: z.number().min(0).max(100).optional(), charging_amps: z.number().positive().optional(), cabin_temp_c: z.number().min(15).max(28).optional(), cop_temp: z.number().int().min(1).max(3).optional(), speed_limit_mph: z.number().positive().optional(), speed_limit_pin: z.string().optional(), cabin_overheat_on: z.boolean().optional(), fan_only: z.boolean().optional(), wait_for_completion: z.boolean().optional() }).strict();
const endpointMap: Partial<Record<(typeof operations)[number], string>> = { set_temperature: "set_temperatures" };
export function buildCommand(operation: string, input: unknown, confirm?: boolean) {
  ...
  const map: Record<string, string> = { charge_limit_percent: "percent", charging_amps: "amps", cabin_temp_c: "temperature", cop_temp: "cop_temp", speed_limit_mph: "mph", speed_limit_pin: "pin", cabin_overheat_on: "on", fan_only: "fan_only", wait_for_completion: "wait_for_completion" };
  for (const [key, value] of Object.entries(params)) if (value !== undefined) body[map[key]] = value;
  return { path: `/command/${endpointMap[parsedOperation] ?? parsedOperation}`, body, requiresConfirmation: !safe.has(parsedOperation), redactions: ["speed_limit_pin"] };
}
